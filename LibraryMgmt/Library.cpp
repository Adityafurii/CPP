...
void BOOK::booklist(int i)
{
    int b,r=0;
    system("cls");
    b=branch(i);
    system("cls");
    ifstream intf("book_db.dat",ios::binary);
    if(!intf)
        cout<<"\n\t\tFile Not Found.";
    else
    {
...
void BOOK::modify()
{
    ...
    if(i==1)
    {
        system("cls");
        b=branch(2);
        ifstream intf1("book_db.dat",ios::binary);
        if(!intf1)
        {
            cout<<"\n\t\tFile Not Found\n";
            cout<<"\n\t\tPress any key to continue.....";
        ...
        if(i==1)
        {
            ...
            cin.getline(st1,100);
            system("cls");
            fstream intf("book_db.dat",ios::in|ios::out|ios::ate|ios::binary);
            intf.seekg(0);
            intf.read((char*)this,sizeof(*this));
            while(!intf.eof())
            {
    ...
    else if(i==2)
    {
        ...
        system("cls");
        getdata();
        ofstream outf("book_db.dat",ios::app|ios::binary);
...
void BOOK::issue()
{
    ...
    system("cls");
    cout<<"\n\t\t->Please Choose one option :-\n";
    cout<<"\n\t\t1.Issue Book\n\n\t\t2.View Issued Book\n\n\t\t3.Search user who issued books\n\n\t\t4.Re-Issue Book\n\n\t\t5.Return Book\n\n\t\t6.Go back to menu\n\n\t\tEnter Your Choice : ";
    cin>>i;
    fflush(stdin);
    if(i==1)
    {
        ...
        cout<<"\n\t\tEnter date : ";
        cin>>q>>B>>p;
        ofstream outf("user_db.dat",ios::binary|ios::app);
        outf.write((char*)this,sizeof(*this));
        outf.close();
        cout<<"\n\n\t\tIssue Successfully.\n";
    }
    ...
    {
        ifstream intf("user_db.dat",ios::binary);
        system("cls");
        cout<<"\n\t\t->The Details are :-\n";
        intf.read((char*)this,sizeof(*this));
        i=0;
    ...
    else if(i==4)
    {
        ...
        cout<<"\n\t\tEnter Book's ID : ";
        cin.getline(st1,20);
        fstream intf("user_db.dat",ios::in|ios::out|ios::ate|ios::binary);
        intf.seekg(0);
        intf.read((char*)this,sizeof(*this));
        while(!intf.eof())
        {
...
void BOOK::fine(int d,int m,int y,int dd,int mm,int yy)
{
    long int n1,n2;
    int years,l,i;
...
void BOOK::der(char st[],int b,int x)
{
    int i,cont=0;
    fstream intf("book_db.dat",ios::in|ios::out|ios::ate|ios::binary);
    intf.seekg(0);
